    ...
    async handleMessageDeleted(data) {
        try {
            ...
            if (!deletedMessageInfo && chat.length > 0) {
                ...
                // 优先级1：基于聊天模式推断
                // 关键逻辑：用户消息和AI消息通常是成对出现的
                // - 如果最后一条是用户消息，说明删除的是AI消息（重新生成场景）
                // - 如果最后一条是AI消息，说明删除的是用户消息（删除/编辑场景）
                if (chat.length > 0) {
                    const lastMessage = chat[chat.length - 1];
                    if (lastMessage?.is_user) {
                        // 最后一条是用户消息 → 删除的是AI消息
                        console.log('[EventSystem] 🧠 推断：最后一条是用户消息，删除的是AI消息（重新生成场景）');
                        ...
                        // 最后一条是AI消息 → 删除的是用户消息
                        console.log('[EventSystem] 🧠 推断：最后一条是AI消息，删除的是用户消息（删除/编辑场景）');
                        inferredDeletedMessage = {
                            index: chat.length, // 被删除的用户消息原本在末尾
                            isUser: true, // 🔧 修复：推断为用户消息
                            message: null,
                            inference: 'user_message_deletion'
                        };
                    }
                }
