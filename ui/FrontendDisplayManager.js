...
export class FrontendDisplayManager {
    ...
    async init() {
        ...
        try {
            ...
            // 如果明确设置为enabled=true，或者有配置项，则启用
            if (saved.enabled === true || hasConfiguredItems) {
                console.log('[FrontendDisplayManager] 🔄 自动启用前端显示 (enabled=' + saved.enabled + ', hasItems=' + hasConfiguredItems + ')');
                // 确保enabled状态被保存
                if (saved.enabled !== true) {
                    const updated = { ...saved, enabled: true };
                    this.saveFrontendDisplayConfig(updated);
                    console.log('[FrontendDisplayManager] 💾 已更新启用状态到配置');
                }
                await this.setEnabled(true);
                console.log('[FrontendDisplayManager] ✅ 前端显示已启用');
            } else {
                console.log('[FrontendDisplayManager] ⚠️ 前端显示保持禁用状态 (enabled=' + saved.enabled + ', hasItems=' + hasConfiguredItems + ')');
            }
